            ...
            switch (key) {
                ...
                case "MSG":
                    ...
                    if (valueStr.trim().isEmpty()) {
                        String errorText = "Empty message is not allowed";
                        byte[] errResponse = KLVExample.encodeKLV("ERR", errorText.getBytes(StandardCharsets.UTF_8));
                ...
                case "PFP":
                    String indexStr = new String(value, StandardCharsets.UTF_8);
                    int indx;
                    try {
                        indx = Integer.parseInt(indexStr);
                    } catch (NumberFormatException e) {
                        String errText = "Invalid pfp index: " + indexStr;
                        byte[] errResponse = KLVExample.encodeKLV("ERR", errText.getBytes(StandardCharsets.UTF_8));
                        return new ResponseResult(errResponse, 400);
                    }

                    String pfpP = username + ":" + indx;
                    byte[] pfpResponse = KLVExample.encodeKLV("PFP", pfpP.getBytes(StandardCharsets.UTF_8));
                    return  new ResponseResult(pfpResponse, 200);
